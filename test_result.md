...
agent_communication:
    - agent: "main"
    - message: "✅ OPRAVENO: Drag restore podprojektu (projectNode) z Koše do Workspace. Problém: V Canvas.jsx handleTrashContentRestore() chyběl case pro itemType='subproject'. OPRAVA: Přidán kompletní handler pro 'subproject' case s flow: 1) Obnovení dat do localStorage (project_world_X) 2) Otevření/zaostření modulu Projekty přes addModule()/bringToFront() 3) Dispatch steward-project-element-restored eventu s projectId a nodePath pro navigaci na obnovený podprojekt. Používá identický pattern jako project_element restore - NEmanipuluje přímo WindowManager, pouze volá standardní openModule API."
    - agent: "main"
